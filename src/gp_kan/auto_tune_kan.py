...
class GPKANAutoTuner:
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            global_length_scale = trial.suggest_float("global_length_scale", 0.01, 1.0)
            min_length_scale = trial.suggest_float("min_length_scale", 0.01, 1.0)

            global_covariance_scale = trial.suggest_float(
                "global_covariance_scale", 0.001, 1.0
            )
            min_covariance_scale = trial.suggest_float(
                "min_covariance_scale", 0.001, 1.0
            )
            ...
            min_var = trial.suggest_float("min_var", 0.001, 0.1)
            ...
            cv_results = self._cross_validate_gpkan(
                config, hidden_sizes, n_splits=3
            )  # Reduced for speed
