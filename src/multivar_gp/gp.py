...
class GP:
    ...
    def __init__(self, config: Any, sigma: np.ndarray) -> None:
        ...
        if self.use_sparse:
            self.gp_impl: Union[DenseGP, SparseGP] = SparseGP(config, sigma)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "GP":
        if self.use_sparse and isinstance(self.gp_impl, SparseGP):
    ...
    def eval_fit(
        ...
        if hasattr(self.gp_impl, "eval_fit"):
            ...
            # SparseGP doesn't have eval_fit, return default values
            return 0.0, 0.0, 0.0
