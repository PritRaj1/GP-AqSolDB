...
def test_gpu_layer_init():
    ...
    try:
        ...
        for param_name, param_value in params.items():
            assert hasattr(
                param_value, "device"
            ), f"Parameter {param_name} should have device attribute"
...
def test_jit_forward():
    ...
    print(
        f"JIT compilation successful. Compilation: {compilation_time:.4f}s, "
...
def test_jit_loglikelihood():
    ...
    print(
        f"JIT loglikelihood successful. Compilation: {compilation_time:.4f}s, "
        f"Execution: {execution_time:.4f}s"
    )
...
def test_memory_efficiency():
    ...
    cpu_layer = DenseGPLayer(
        input_size=10, output_size=5, config=cpu_config, key=jax.random.PRNGKey(42)
    )
    ...
    try:
        gpu_layer = DenseGPLayer(
            input_size=10, output_size=5, config=gpu_config, key=jax.random.PRNGKey(42)
        )
        ...
        if (
            ...
            pytest.skip(
                "GPU memory test failed - likely due to insufficient GPU memory"
            )
        else:
...
def test_device_transfer():
    cpu_dist = NormalDist(jnp.array([[0.0, 1.0, -0.5]]), jnp.array([[0.1, 0.2, 0.15]]))
