...
def test_conf():
    ...
    assert settings["use_parallel"] is False
...
def test_conf_load():
    ...
    assert settings["use_parallel"] is True
    assert settings["n_jobs"] == 4
    assert settings["chunk_size"] == 500
    assert settings["use_gpu"] is False
    assert settings["min_size_for_parallel"] == 300
...
def test_gpu():
    ...
    try:
        ...
        gpu_related = any(
            keyword in error_msg for keyword in ["gpu", "cuda", "device", "memory"]
        )
        if not gpu_related:
            raise
...
def test_chunks():
    ...
    for chunk_size in [100, 500, 1000]:
        configure_parallel_settings(
            use_parallel=True, use_gpu=False, n_jobs=2, chunk_size=chunk_size
        )
...
def test_min_size_for_parallel():
    ...
    configure_parallel_settings(
        use_parallel=True, use_gpu=False, n_jobs=2, min_size_for_parallel=100
    )
...
def test_different_n_jobs(n_jobs):
    ...
    configure_parallel_settings(use_parallel=True, use_gpu=False, n_jobs=n_jobs)
...
def test_memory_efficiency():
    ...
    configure_parallel_settings(
        use_parallel=True, use_gpu=False, n_jobs=2, chunk_size=500
    )
...
if __name__ == "__main__":
    ...
    test_data = create_test_data(500, 3)

    test_sequential_vs_parallel("RBF", test_data)
    test_sequential_vs_parallel("RQ", test_data)
