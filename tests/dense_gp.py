...
def test_gp_univariate_sigma(return_data=False):
    ...
    X_train, y_train = get_data(
        num_points=20, noise=True, noise_std=0.1, x_range=(0, 10)
    )
    X_test = np.linspace(0, 10, 100).reshape(-1, 1)
...
def test_gp_rational_quadratic_kernel(return_data=False):
    ...
    X_train, y_train = get_data(
        num_points=25, noise=True, noise_std=0.1, x_range=(0, 10)
    )
    X_test = np.linspace(0, 10, 100).reshape(-1, 1)
...
def visualize_gp_results():
    ...
    X_train_2d, y_train_2d, X_test_2d, y_pred_grid, y_std_grid, X1, X2 = (
        test_gp_multivariate_sigma(return_data=True)
    )
    ...
    axes[0, 1].set_title(
        r"Mean Prediction $\mathbb{E}[f(\mathbf{x})]$", fontweight="bold", pad=15
    )
    ...
    axes[1, 0].set_title(
        r"Uncertainty $\sqrt{\text{Var}[f(\mathbf{x})]}$", fontweight="bold", pad=15
    )
    ...
    X_train_rq, y_train_rq, X_test_rq, y_pred_rq, y_std_rq = (
        test_gp_rational_quadratic_kernel(return_data=True)
    )

    sns.scatterplot(
        x=X_train_rq.flatten(),
        y=y_train_rq,
        color="red",
        s=30,
        label="Training Data",
    ...
    plt.savefig(
        "tests/figures/gp_test_results.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
...
if __name__ == "__main__":
    ...
    X_train_1d, y_train_1d = get_data(
        num_points=20, noise=True, noise_std=0.1, x_range=(0, 10)
    )
    data_1d = (X_train_1d, y_train_1d)

    np.random.seed(42)
    X_train_2d = np.random.uniform(0, 5, (30, 2))
    y_train_2d = np.sin(X_train_2d[:, 0]) * np.exp(
        X_train_2d[:, 1] / 5
    ) + np.random.normal(0, 0.1, 30)
    data_2d = (X_train_2d, y_train_2d)

    test_gp_basic_functionality("RBF", 1.0, data_1d)
    test_gp_basic_functionality("RQ", 1.0, data_1d)
    test_gp_sigma_types("univariate", data_1d, data_2d)
    test_gp_sigma_types("multivariate", data_1d, data_2d)
    test_uncertainty_behavior()
    test_uncertainty_distance_relationship(data_1d)
    test_gp_fit_attributes(data_1d)
