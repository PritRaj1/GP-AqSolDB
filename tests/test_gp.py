...
def test_unified_gp_switching():
    ...
    assert not gp_dense.use_sparse, "use_sparse should be False for dense GP"
    ...
    assert gp_sparse.use_sparse, "use_sparse should be True for sparse GP"
    ...
    assert (
        gp_sparse.get_model_complexity() > gp_dense.get_model_complexity()
    ), "Sparse GP should have higher complexity"
...
def test_different_inducing_methods(method):
    ...
    assert (
        model_info["inducing_method"] == method
    ), f"Inducing method should be {method}"
    assert (
        model_info["num_inducing"] == 30
    ), "Number of inducing points should match config"
...
def test_uncertainty_quantification():
    ...
    assert np.all(
        y_std_sparse > 0
    ), "All uncertainties should be positive for sparse GP"
    assert (
        np.mean(y_std_sparse) > 0
    ), "Mean uncertainty should be positive for sparse GP"
...
def test_config_loading():
    ...
    config["SPARSE"] = {"use_sparse": "true", "inducing_method": "uniform"}

    sigma = np.array([1.0, 1.0])

    gp = GP(config, sigma)
    assert (
        gp.model_type == "sparse"
    ), "Model type should be 'sparse' for use_sparse=true"
    assert gp.use_sparse is True, "use_sparse should be True"
    ...
    assert model_info["model_type"] == "sparse", "Model info should indicate sparse GP"
    assert (
        model_info["inducing_method"] == "uniform"
    ), "Inducing method should be uniform"
...
def test_model_complexity():
    ...
    config_sparse = create_config(
        use_sparse=True, num_inducing=20, inducing_method="random"
    )
    gp_sparse = GP(config_sparse, sigma)
    ...
    assert (
        sparse_complexity > dense_complexity
    ), "Sparse GP should have higher complexity than dense GP"
...
def test_fit_predict_cycle():
    ...
    assert len(y_pred_dense) == len(
        X_test
    ), "Prediction length should match test data length"
    assert np.all(np.isfinite(y_pred_dense)), "Predictions should be finite"

    # Test sparse GP
    config_sparse = create_config(
        use_sparse=True, num_inducing=10, inducing_method="random"
    )
    gp_sparse = GP(config_sparse, sigma)
    gp_sparse.fit(X_train, y_train)
    y_pred_sparse = gp_sparse.predict(X_test)

    assert len(y_pred_sparse) == len(
        X_test
    ), "Prediction length should match test data length"
