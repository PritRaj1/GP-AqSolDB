...
def test_sparse_vs_full_gp():
    ...
    # Performance assertions - allow for very fast operations (otherwise time = 0.0)
    ...
    assert (
        sparse_info["num_inducing"] == 50
    ), "Number of inducing points should match config"
    assert sparse_info["num_training"] == len(
        X_train
    ), "Number of training points should match data"
    assert (
...
def test_different_inducing_points(num_inducing):
    ...
    assert (
        fit_time >= 0
    ), f"Training time should be non-negative for {num_inducing} inducing points"
    assert mse > 0, f"MSE should be positive for {num_inducing} inducing points"
    assert (
        sparse_info["num_inducing"] == num_inducing
    ), f"Number of inducing points should be {num_inducing}"
...
def test_sparse_gp_with_different_kernels(kernel_type):
    ...
    ), f"Full GP predictions should be finite for {kernel_type} kernel"
    assert np.all(
        np.isfinite(y_pred_sparse)
